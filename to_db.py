...
def mongod_trial(dir_out="analysis/mdb_test_out"):
    # dbの中にあるリストを結合
    pipeline = [
        {
            "$lookup": {
        ...
        {
            "$project": {"_id": 1, "log_name": 1, "item_name": 1, "input_order": 1, "data_type": "$exp_item.data_type",
                         "scale": "$exp_item.scale", "index_name": 1,
                         "collection_name": "$exp_item.collection_name", "index_number": 1,
                         }
        }]
